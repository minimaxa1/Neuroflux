...
# --- LlamaIndex and Vector Store (Using Qdrant locally) ---
from llama_index.core import VectorStoreIndex, SimpleDirectoryReader, StorageContext, Settings, Document
from llama_index.core.query_engine import RetrieverQueryEngine
from llama_index.vector_stores.qdrant import QdrantVectorStore # Re-added QdrantVectorStore import
import qdrant_client # Re-added Qdrant client import
# Removed AsyncQdrantClient import - not needed for local mode
...
# Removed SimpleVectorStore import - no longer used for RAG
...
class AppSettings(BaseSettings):
    ...
    # Keeping PERSIST_DIR for generic storage, but Qdrant will use its own dedicated directory
    PERSIST_DIR: str = "storage" 
    
    QDRANT_COLLECTION_NAME: str = "neuroflux_ghostwriter_v21"
    # NEW: Qdrant local persistence directory
    QDRANT_LOCAL_DATA_DIR: str = "qdrant_local_data" # Directory for Qdrant's local files

    # NEW: Directory to save generated HTML reports
    REPORT_SAVE_DIR: str = "generated_reports"
...
class ResourceManager:
    def __init__(self, config: AppSettings):
        ...
        # Qdrant client for local mode (does not require AsyncQdrantClient)
        ...
        # Store a reference to the Qdrant vector store
        self._qdrant_vector_store: Optional[QdrantVectorStore] = None 
    ...
    async def startup(self):
        ...
        # Ensure necessary directories exist, including the Qdrant local data directory and report save directory
        for path in [self.config.PERSIST_DIR, self.config.KNOWLEDGE_BASE_DIR, self.config.QDRANT_LOCAL_DATA_DIR, self.config.REPORT_SAVE_DIR]: 
            os.makedirs(path, exist_ok=True)
        
        Settings.embed_model = FastEmbedEmbedding(model_name=self.config.EMBEDDING_MODEL_NAME)
        
        # Initialize Qdrant client in local, file-based persistence mode
        try:
            self.qdrant_client = qdrant_client.QdrantClient(
                location=self.config.QDRANT_LOCAL_DATA_DIR, # This tells Qdrant to use local files
            )
            logger.info(f"✅ Qdrant client initialized in local mode, data stored at: {self.config.QDRANT_LOCAL_DATA_DIR}")
        except Exception as e:
            logger.error(f"FATAL: Could not initialize local Qdrant client at {self.config.QDRANT_LOCAL_DATA_DIR}. Error: {e}", exc_info=True)
    ...
    async def shutdown(self):
        ...
        # For Qdrant local mode, no explicit close() method is typically needed or available on the client object itself.
        # The data is flushed to disk by the embedded server.
        logger.info("✅ All resource managers shut down.")

    async def load_rag_engine(self):
        if not self.qdrant_client:
            self.indexing_status = {"status": IndexingStatus.ERROR, "message": "Qdrant client not initialized. RAG 'Soul' is dormant."}
            logger.error("❌ Local Qdrant client not initialized. Cannot load RAG engine.")
            return

        try:
            # Check if the collection exists in the local Qdrant instance
            collections_response = self.qdrant_client.get_collections()
            collection_exists = any(
                c.name == self.config.QDRANT_COLLECTION_NAME
                for c in collections_response.collections
            )
            
            if collection_exists:
                self._qdrant_vector_store = QdrantVectorStore(
                    client=self.qdrant_client, # Use the local client
                    collection_name=self.config.QDRANT_COLLECTION_NAME,
                    # enable_hybrid=True # Can be enabled if needed, but might depend on local mode support
                )
                # Load the index from the existing Qdrant vector store
                self.rag_query_engine = VectorStoreIndex.from_vector_store(self._qdrant_vector_store).as_query_engine(
                    similarity_top_k=settings.RERANK_TOP_N * 3
                )
                self.indexing_status = {"status": IndexingStatus.IDLE, "message": "RAG 'Soul' loaded from local Qdrant files."}
                logger.info("✅ RAG 'Soul' loaded from local Qdrant files.")
            else:
                self.indexing_status = {"status": IndexingStatus.IDLE, "message": "Local Qdrant collection not found. RAG 'Soul' dormant (requires indexing)."}
                logger.warning("⚠️ Local Qdrant collection not found. RAG 'Soul' dormant (requires indexing).")
                self.rag_query_engine = None
        except Exception as e:
            self.indexing_status = {"status": IndexingStatus.ERROR, "message": f"Error loading RAG from local Qdrant: {e}"}
            logger.error(f"❌ Error loading RAG from local Qdrant: {e}", exc_info=True)
            self.rag_query_engine = None
...
async def agent_event_generator(request: BaseModel, rm: ResourceManager) -> AsyncIterator[str]:
    ...
    full_html_content_chunks = [] # Initialize list to store HTML chunks
    saved_report_path = None # To store the path of the saved report
    ...
    try:
        ...
        **Instructions for Report Generation:**
        ...
        async with rm.http_client.stream("POST", f"{settings.OLLAMA_API_BASE}/api/chat", json=payload) as response:
            ...
            async for line in response.aiter_lines():
                ...
                if content := chunk.get("message", {}).get("content"):
                    full_html_content_chunks.append(content) # Accumulate chunks
                    yield f'data: {json.dumps({"type": "synthesis_token", "content": content})}\n\n'
                if chunk.get("done"): break

        # --- After streaming is complete, save the full report ---
        full_html_report = "".join(full_html_content_chunks)

        # Sanitize title for a valid filename
        report_title = intelligence_briefing.get('title', 'Generated Report')
        sanitized_title = re.sub(r'[^\w\s-]', '', report_title).strip().replace(' ', '_')
        sanitized_title = re.sub(r'[-_]+', '-', sanitized_title) # Replace multiple dashes/underscores with single dash
        if not sanitized_title:
            sanitized_title = f"unnamed_report_{uuid.uuid4().hex[:8]}"

        filename = f"{sanitized_title}.html"
        saved_report_path = os.path.join(settings.REPORT_SAVE_DIR, filename)

        try:
            with open(saved_report_path, "w", encoding="utf-8") as f:
                f.write(full_html_report)
            logger.info(f"Report saved to {saved_report_path}")
            yield f'data: {json.dumps({"type": "log", "content": f"Report saved to: {saved_report_path}", "class": "success"})}\n\n'
        except Exception as file_e:
            logger.error(f"Failed to save report to file {saved_report_path}: {file_e}", exc_info=True)
            yield f'data: {json.dumps({"type": "error", "content": f"Failed to save report: {file_e}"})}\n\n'
            saved_report_path = None # Clear path if save failed
        ...
        # Final complete message, including the path if available
        final_message = "Stream ended."
        if saved_report_path:
            final_message += f" Report available at: {saved_report_path}"
        yield f'data: {json.dumps({"type": "complete", "content": final_message, "file_path": saved_report_path})}\n\n' # Pass file_path in complete event
...
async def start_indexing_endpoint(background_tasks: BackgroundTasks, rm: ResourceManager = Depends(get_resource_manager)):
    ...
    async def build_index_task(rm_instance: ResourceManager):
        ...
        # Check Qdrant client availability before proceeding
        if not rm_instance.qdrant_client:
            rm_instance.indexing_status = {"status": IndexingStatus.ERROR, "message": "Local Qdrant client not available for indexing. Please check initialization."}
            logger.error("❌ Local Qdrant client not available for indexing.")
            return

        try:
            knowledge_dir = settings.KNOWLEDGE_BASE_DIR
            if not os.path.exists(knowledge_dir) or not os.listdir(knowledge_dir):
                rm_instance.indexing_status["message"] = "Knowledge base directory is empty. No documents to index."; return
            
            # Load documents normally
            documents = SimpleDirectoryReader(knowledge_dir, recursive=True).load_data()
            if not documents: rm_instance.indexing_status["message"] = "No supported documents found in knowledge base."; return

            nodes = documents 
            
            if not nodes: rm_instance.indexing_status["message"] = "No nodes generated from documents (check document format/content)."; return

            rm_instance.indexing_status["message"] = f"Found {len(documents)} documents. Building vector index in local Qdrant..."
            
            # --- Qdrant-specific indexing logic (for local mode) ---
            # It's good practice to delete the collection before a full re-index
            # to ensure fresh data and avoid duplicates or stale entries.
            logger.info(f"Attempting to delete existing local Qdrant collection '{settings.QDRANT_COLLECTION_NAME}' for fresh indexing.")
            try:
                # Use the local QdrantClient directly for collection management
                rm_instance.qdrant_client.delete_collection(collection_name=settings.QDRANT_COLLECTION_NAME, timeout=60)
            except Exception as e:
                # Log a warning if deletion fails (e.g., collection might not exist yet)
                logger.warning(f"Could not delete local Qdrant collection '{settings.QDRANT_COLLECTION_NAME}' (might not exist or other error): {e}")

            # Initialize QdrantVectorStore, linking to the local Qdrant client
            rm_instance._qdrant_vector_store = QdrantVectorStore(
                client=rm_instance.qdrant_client, # Use the local client
                collection_name=settings.QDRANT_COLLECTION_NAME,
                # enable_hybrid=True # Can be enabled if needed, but might depend on local mode support
            )
            store_type_message = "Qdrant (Local File Mode)"

            # Create StorageContext with the QdrantVectorStore
            storage_context = StorageContext.from_defaults(vector_store=rm_instance._qdrant_vector_store)
            
            # Build the index using the documents and the Qdrant storage context
            # This will write the embeddings to the local Qdrant data directory
            ...
            rm_instance.rag_query_engine = VectorStoreIndex.from_vector_store(rm_instance._qdrant_vector_store).as_query_engine(
                similarity_top_k=settings.RERANK_TOP_N * 3
            )
            
            rm_instance.indexing_status = {"status": IndexingStatus.IDLE, "message": f"Index built successfully ({store_type_message})."}
            logger.info(f"✅ RAG 'Soul' initialized ({store_type_message}).")
        except Exception as e:
            rm_instance.indexing_status = {"status": IndexingStatus.ERROR, "message": f"Indexing failed: {e}. Check local Qdrant data directory permissions."}
